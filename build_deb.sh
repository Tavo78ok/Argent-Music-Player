...
cp argent_music_player.py "$DEB_ROOT/usr/share/$PKG/"

cat > "$DEB_ROOT/usr/bin/$PKG" << 'EOF'
#!/bin/bash
exec python3 /usr/share/argent-music-player/argent_music_player.py "$@"
EOF
...
Keywords=music;audio;player;mp3;flac;opus;lrc;letras;argent;
...
Maintainer: Andrés <andres@localhost>
Installed-Size: $(du -sk "$DEB_ROOT" | cut -f1)
Description: Argent Music Player
 Reproductor de música local con soporte para MP3, FLAC, OGG,
 Opus, M4A y más formatos.
 .
