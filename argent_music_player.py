#!/usr/bin/env python3
"""Argent Music Player v1.1.0 - GTK4 + libadwaita + GStreamer + SQLite"""
...
DB_PATH   = os.path.expanduser("~/.local/share/argent-music-player/library.db")
CONF_PATH = os.path.expanduser("~/.local/share/argent-music-player/config.json")
    ...
    def __init__(self, win):
        ...
        try:
            ...
            Gio.bus_own_name_on_connection(
                self._conn,"org.mpris.MediaPlayer2.ArgentMusicPlayer",
        ...
        try:
            if iface=="org.mpris.MediaPlayer2":
                d={"CanQuit":GLib.Variant("b",True),"CanRaise":GLib.Variant("b",True),
                   ...
                   "Identity":GLib.Variant("s","Argent Music Player"),
...
class LyricsManager:
    ...
    def _try_online(self,song):
        ...
        hdrs={
            "User-Agent":"ArgentMusicPlayer/1.1 (https://github.com/Tavo78ok/argent-music-player)",
            "Accept":"application/json",
            "Lrclib-Client":"ArgentMusicPlayer v1.1 (https://github.com/Tavo78ok/argent-music-player)",
        }
...
# ══ Main Window ════════════════════════════════════════════════════════
class ArgentMusicPlayer(Adw.ApplicationWindow):
    def __init__(self,app):
        super().__init__(application=app)
        self.set_title("Argent Music Player")
        ...
        # Aplicar tema guardado (si el usuario eligió uno explícitamente antes)
        self._cfg=load_config()
        saved_scheme=self._cfg.get("color_scheme")
        if saved_scheme=="dark":
            Adw.StyleManager.get_default().set_color_scheme(Adw.ColorScheme.FORCE_DARK)
        elif saved_scheme=="light":
            Adw.StyleManager.get_default().set_color_scheme(Adw.ColorScheme.FORCE_LIGHT)
        # si no hay preferencia guardada, se mantiene DEFAULT (sigue al sistema)
    ...
    def _build_ui(self):
        hdr=Adw.HeaderBar()
        self._wtitle=Adw.WindowTitle(title="Argent Music Player",subtitle="Biblioteca")
    ...
    def _on_toggle_theme(self,_):
        ...
        if current==Adw.ColorScheme.FORCE_DARK:
            ...
            self._cfg["color_scheme"]="light"
        else:
            mgr.set_color_scheme(Adw.ColorScheme.FORCE_DARK)
            self._cfg["color_scheme"]="dark"
...
# ══ Application ════════════════════════════════════════════════════════
class ArgentMusicApp(Adw.Application):
    def __init__(self):
        # prgname debe configurarse ANTES de crear la app
        # Es lo que GTK4 usa como WM_CLASS instance name
        GLib.set_prgname("argent-music-player")
        GLib.set_application_name("Argent Music Player")
        super().__init__(application_id="io.github.Tavo78ok.ArgentMusicPlayer",flags=Gio.ApplicationFlags.FLAGS_NONE)
    ...
    def _on_activate(self,_):
        self.win=ArgentMusicPlayer(self); self.win.present()

    def _about(self,*_):
        Adw.AboutDialog(
            application_name="Argent Music Player",application_icon="argent-music-player",
            version="1.1.0",developer_name="Andrés",
            comments="Reproductor de música local con ecualizador, fundido entre canciones, colas múltiples, letras sincronizadas, mini reproductor e integración MPRIS2.",
            license_type=Gtk.License.GPL_3_0).present(self.win)

def main():
    app=ArgentMusicApp(); sys.exit(app.run(sys.argv))
